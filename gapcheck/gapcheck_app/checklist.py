...
SOURCE OF TRUTH: ../../agent/checklist.py
This is a verbatim copy. A FAB packages only its own app directory, so the
SuperNode cannot import from the repository root. If you edit one, edit both.

This is the ONLY thing that travels outward from the coordinator, and in the
federated build that is now literally true: the ServerApp serialises this
module and sends it to every node. The ClientApp carries no copy of its own -
it evaluates against whatever rubric it was handed.
